...
function createDatabaseDouble() {
  ...
  return {
      ...
      return {
        ...
        async first() {
          ...
          if (sql.includes('SELECT id FROM replies')) {
            return state.replies.find((reply) => reply.userId === this.args[0] && reply.letterDate === this.args[1] && reply.spiritId === this.args[2]) || null
          }
          if (sql.includes('chat_json AS chatJson')) return { chatJson: '{"sharedMemories":[{"content":"用户喜欢安静的早晨"}]}' }
          return null
        },
        async all() {
          if (sql.includes('GROUP BY user_id, local_date')) return { results: [{ userId: 'user-1', localDate: '2026-07-18', spiritId: 'lino' }] }
    ...
    async batch(statements) {
      ...
      if (insert && !state.replies.some((reply) => reply.userId === 'user-1' && reply.letterDate === '2026-07-18' && reply.spiritId === 'lino')) {
        state.replies.push({ userId: 'user-1', letterDate: '2026-07-18', spiritId: insert.args[4], content: insert.args[5] })
      }
...
test('chat selects the requested spirit and receives summary-level shared memory', async () => {
  const originalFetch = globalThis.fetch
  let providerPayload
  globalThis.fetch = async (_url, options) => {
    providerPayload = JSON.parse(options.body)
    return new Response('data: {"choices":[{"delta":{"content":"先把今晚放慢一点。"}}]}\n\ndata: [DONE]\n\n', {
      status: 200,
  ...
  try {
    const response = await worker.fetch(new Request('https://example.com/api/chat', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: '今晚有点睡不着。' }],
        memoryContext: [{ spiritId: 'momo', title: '最近的节奏', content: '用户最近希望晚上少一些催促。' }],
      }),
    }), { DEEPSEEK_API_KEY: 'test-key', DEEPSEEK_MODEL: 'deepseek-v4-flash' })

    expect(response.status).toBe(200)
    expect(await response.text()).toContain('先把今晚放慢一点')
    expect(providerPayload.messages[0].content).toContain('月影国的梦境精灵')
    expect(providerPayload.messages[0].content).toContain('用户最近希望晚上少一些催促')
    expect(providerPayload.messages[0].content).toContain('不要声称看过其他精灵的完整原话')
  } finally {
    globalThis.fetch = originalFetch
  }
})
